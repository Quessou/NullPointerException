...
class RegisterController extends grails.plugin.springsecurity.ui.RegisterController {
	...
	def register(RegisterCommand registerCommand) {
		...
		//def
		SecureUser user = uiRegistrationCodeStrategy.createUser(registerCommand)  // Create an user
		def role = null;
		role = SecureRole.findByAuthority('ROLE_USER')
		println role ?: "Role not found"
		...
		user.accountLocked=false;	//	sendVerifyRegistrationMail registrationCode, user, registerCommand.email
		user.save();
				SecureUserSecureRole.create user, role
	...
	passwordValidationRegex = conf.ui.password.validationRegex ?: '^.*(?=.*\\d)(?=.*[a-z])(?=.*[!@#$%^&]).*$'
...
class ForgotPasswordCommand extends grails.plugin.springsecurity.ui.ForgotPasswordCommand {
